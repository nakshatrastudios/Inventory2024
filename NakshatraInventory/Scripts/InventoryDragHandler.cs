...
public class InventoryDragHandler : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler
{
    ...
    private void Awake()
    {
        ...
        {
            if (inventory.nextPageButton == null)
            {
                Debug.LogError("NextPageButton reference is not set.");
            }

            if (inventory.previousPageButton == null)
            {
                Debug.LogError("PreviousPageButton reference is not set.");
            }
        }
    }
        ...
        if (slot.item != null)
        {
            ...
            if (inventory != null && inventory.nextPageButton != null && inventory.previousPageButton != null)
            {
                ...
                if (!isChangingPage && RectTransformUtility.RectangleContainsScreenPoint(nextButtonRect, Input.mousePosition, canvas.worldCamera))
                {
                    ...
                    isChangingPage = true;
                    inventory.NextPage();
                    rectTransform.anchoredPosition = originalPosition; // Reset position to original
